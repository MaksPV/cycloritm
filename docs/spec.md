...
- **разлёт шаблона `span`** — максимальное суммарное смещение событий, достижимое из начала шаблона. Пустой шаблон: `span = 0`. Рекурсия запрещена, поэтому разлёт всегда конечен и вычислим статически:
